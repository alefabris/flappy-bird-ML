...
class Pipe():
    ...
    def move(self): # move pipe based on vel
        self.x -= self.VEL

    def draw(self, win): # draw both the top and bottom of the pipe
        win.blit(self.PIPE_TOP, (self.x, self.top)) # draw top
        win.blit(self.PIPE_BOTTOM, (self.x, self.bottom)) # draw bottom


    def collide(self, bird, win): #  returns if a point is colliding with the pipe
...
class Base:
    ...
    def move(self): # move floor so it looks like its scrolling
        self.x1 -= self.VEL
        self.x2 -= self.VEL
    ...
    def draw(self, win): # Draw the floor, combosed by two images that move together
        win.blit(self.IMG, (self.x1, self.y))
        win.blit(self.IMG, (self.x2, self.y))

# Rotate a surface and blit it to the window
def blitRotateCenter(surf, image, topleft, angle): # Rotate a surface and blit it to the window
...
def eval_genomes(genomes, config): 
    ...
    while run and len(birds) > 0:
        ...
        # break if score gets large enough and save the data
        if score > 20:
